...
router.get('/', async (req, res) => {
    try {
      ...
      `)
        ...
        const configsWithBookings = await Promise.all((configs || []).map(async (config) => {
            let sessionBookings = [];
            // Get the parent booking's session_group_id first
            if (config.parent_booking_id) {
                const { data: parentBooking } = await supabase_1.supabase
                    .from('bookings')
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { is_active, custom_total_sessions, custom_strategy, custom_min_days_between_sessions, custom_max_days_between_sessions, custom_buffer_before_minutes, custom_buffer_after_minutes, custom_session_schedule, status, notes } = req.body;
        if (!(0, uuid_validator_1.isValidUUID)(id)) {
            return res.status(400).json({ error: 'Invalid configuration ID format' });
        }
        const updateData = {};
        ...
        if (custom_total_sessions !== undefined)
            updateData.custom_total_sessions = custom_total_sessions;
        if (custom_strategy !== undefined)
            updateData.custom_strategy = custom_strategy;
        if (custom_min_days_between_sessions !== undefined)
            updateData.custom_min_days_between_sessions = custom_min_days_between_sessions;
        if (custom_max_days_between_sessions !== undefined)
            updateData.custom_max_days_between_sessions = custom_max_days_between_sessions;
        if (custom_buffer_before_minutes !== undefined)
            updateData.custom_buffer_before_minutes = custom_buffer_before_minutes;
        if (custom_buffer_after_minutes !== undefined)
            updateData.custom_buffer_after_minutes = custom_buffer_after_minutes;
        if (custom_session_schedule !== undefined)
            updateData.custom_session_schedule = custom_session_schedule;
        if (status !== undefined)
...
router.post('/', async (req, res) => {
    try {
        const { contact_id, service_id, parent_booking_id, custom_total_sessions, custom_strategy, custom_min_days_between_sessions, custom_max_days_between_sessions, custom_buffer_before_minutes, custom_buffer_after_minutes, custom_session_schedule, notes } = req.body;
