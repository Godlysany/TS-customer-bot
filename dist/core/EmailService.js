...
{{business_name}}`;
    ...
    async sendCancellationEmail(bookingId, contactEmail, cancellationDetails) {
        const html = `
    ...
    async sendReminderEmail(bookingId, contactEmail, reminderDetails) {
        const html = `
    ...
    async sendReviewRequest(bookingId, contactEmail, reviewDetails) {
        const html = `
    ...
    async sendDailySummaryToSecretary(summaryData) {
    ...
    async sendSecretaryBookingNotification(booking, type) {
        const secretaryEmail = await this.settingsService.getSetting('secretary_email');
        if (!secretaryEmail) {
            return;
        }
    ...
    async logEmail(data) {
        await supabase_1.supabase.from('email_logs').insert({
