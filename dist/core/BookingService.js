...
const BusinessHoursService_1 = __importDefault(require("./BusinessHoursService"));
const TeamMemberUnavailabilityService_1 = __importDefault(require("./TeamMemberUnavailabilityService"));
class BookingService {
     ...
     */
    async validateAndPrepare(contactId, conversationId, event, options) {
        ...
        if (options?.teamMemberId) {
            ...
            const schedule = teamMember.availability_schedule?.[dayOfWeek] || [];
            ...
            const isWithinSchedule = schedule.some((window) => {
            ...
            const unavailabilityConflicts = await TeamMemberUnavailabilityService_1.default.getConflicts(options.teamMemberId, actualStartTime, actualEndTime);
            ...
            const { data: teamMemberConflicts } = await supabase_1.supabase
                .from('bookings')
     ...
     */
    async finalizeSideEffects(booking, context) {
        ...
        // Schedule reminders
        await this.reminderService.scheduleAppointmentReminder(booking.id, context.contactId, context.event.startTime);
        // Schedule review request
        await this.reviewService.scheduleReviewRequest(booking.id, context.contactId);
        // Notify secretary
        await this.secretaryService.notifyBookingCreated((0, mapper_1.toSnakeCase)(booking));
     ...
     */
    async rollbackBooking(bookingId, calendarEventId) {
    ...
    async checkConfigurationRestrictions(event, serviceId, alwaysFollowBusinessHours = true) {
        ...
        if (alwaysFollowBusinessHours) {
            ...
            const availableWindows = await BusinessHoursService_1.default.getAvailableWindows(dayOfWeek);
