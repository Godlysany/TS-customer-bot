...
class NoShowService {
    ...
    async markAsNoShow(bookingId, notes) {
        ...
        return {
            tracking: (0, mapper_1.toCamelCase)(tracking),
            suspended: isSuspended,
        };
    }
    async getContactStrikeCount(contactId) {
    ...
    async isContactSuspended(contactId) {
    ...
    async getNoShowHistory(contactId) {
        const { data, error } = await supabase_1.supabase
            .from('no_show_tracking')
            .select('*')
            .eq('contact_id', contactId)
    ...
    async liftSuspension(contactId) {
        await supabase_1.supabase
            .from('no_show_tracking')
    ...
    async resetStrikes(contactId) {
        const { data: latestTracking } = await supabase_1.supabase
            .from('no_show_tracking')
            .select('id')
            .eq('contact_id', contactId)
            .order('created_at', { ascending: false })
...
exports.NoShowService = NoShowService;
exports.default = new NoShowService();
