...
export interface BookingHistoryInsight {
...
export class MultiServiceBookingService {
  ...
  private async getBookingHistory(contactId: string): Promise<BookingHistoryInsight> {
    ...
    const mostFrequent = Object.entries(serviceCounts).sort((a, b) => (b[1] as number) - (a[1] as number))[0];

    const oldestBooking = new Date(bookings[bookings.length - 1].created_at);
    const monthsSinceFirst =
      (Date.now() - oldestBooking.getTime()) / (1000 * 60 * 60 * 24 * 30);
  ...
  private async getFollowUpRecommendations(
    serviceId: string
  ): Promise<ServiceRecommendation[]> {
    const { data: followUpRules, error } = await supabase
  ...
  async getPopularCombinations(
    ...
    const contactServiceMap = new Map<string, Set<string>>();
    const contactsWithCustomerServices = new Set<string>();
    ...
    const complementaryCounts = new Map<string, number>();
  ...
  private async getNewCustomerRecommendations(): Promise<ServiceRecommendation[]> {
  ...
  async formatRecommendationsMessage(
    recommendations: ServiceRecommendation[]
  ): Promise<string> {
  ...
  async calculateMultiServiceSchedule(
    serviceIds: string[],
    startDate: Date
  ): Promise<{
    ...
    totalCost: number;
    endDate: Date;
    schedule: Array<{ serviceId: string; startTime: Date; endTime: Date }>;
  }> {
    ...
    const schedule: Array<{ serviceId: string; startTime: Date; endTime: Date }> = [];
