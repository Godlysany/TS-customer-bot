...
export class AIService {
  ...
  detectServiceFromMessage(
  ...
  ): { service: { id: string; name: string } | null; method: string } {
    ...
    if (intentEntities?.service) {
      const service = services.find(s => 
        s.name.toLowerCase() === intentEntities.service.toLowerCase()
      );
      if (service) {
  ...
  async generateReply(conversationId: string, messageHistory: Message[], currentMessage: string, intent?: string, contactId?: string, intentEntities?: Record<string, any>): Promise<string> {
    try {
...
Respond with JSON only:
{
   ...
   */
  async analyzeConversationSentiment(conversationId: string, messageHistory: Message[], currentMessage?: string): Promise<{
    overallSentiment: 'positive' | 'neutral' | 'negative' | 'frustrated' | 'confused';
    sentimentScore: number; // -1 to 1
    frustrationLevel: number; // 0 to 1
    confusionLevel: number; // 0 to 1
    satisfactionLevel: number; // 0 to 1
    ...
    try {
      ...
      const conversationMessages = messageHistory.slice(-20).map((msg, idx) => 
        `[${msg.direction === 'inbound' ? 'CUSTOMER' : 'BOT'}]: ${msg.content}`
      ).join('\n');
