...
import { NurturingService } from './NurturingService';
...
export class BirthdayScheduler {
  ...
  start(intervalMinutes: number = 1440): void {
    ...
    this.processBirthdayWishes().catch(err => 
      console.error('Error in initial birthday processing:', err)
    );
  ...
  async processBirthdayWishes(): Promise<{ sent: number; failed: number }> {
