...
router.get('/:id/service-history', async (req, res) => {
  try {
    ...
    const formattedHistory = (bookings || []).map((booking: any) => ({
      ...
      cost: booking.payment_amount_chf,
      notes: booking.notes,
...
router.get('/admin/outstanding-balances', authMiddleware, async (req, res) => {
  try {
    // SECURITY: Master-only endpoint for financial data
    if ((req as any).agent?.role !== 'master') {
    ...
    const totalOutstanding = customersWithDetails.reduce(
      (sum, c) => sum + parseFloat(c.outstanding_balance_chf || 0),
      0
    );
