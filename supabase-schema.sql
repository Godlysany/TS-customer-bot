...
-- =====================================================
-- TEAM MEMBERS INFRASTRUCTURE (Added 2025-11-02)
-- =====================================================
-- Support multiple team members with individual calendars within a single calendar provider
...
-- =====================================================
-- BUFFERED CONFLICT DETECTION (Added 2025-11-02)
-- =====================================================
-- Add actual start/end times with buffers for precise conflict detection
...
CREATE INDEX IF NOT EXISTS idx_bookings_team_actual_times ON bookings(team_member_id, actual_start_time, actual_end_time) 
  WHERE team_member_id IS NOT NULL AND status IN ('confirmed', 'pending');
...
-- Backfill existing bookings with buffered times
UPDATE bookings 
SET 
  actual_start_time = start_time - (COALESCE(buffer_time_before, 0) || ' minutes')::INTERVAL,
  actual_end_time = end_time + (COALESCE(buffer_time_after, 0) || ' minutes')::INTERVAL
