...
- **Phase 4 Bot Configuration Restructure - 100% COMPLETE (October 22, 2025)**: Production-ready restructure of Bot Configuration UI and backend to eliminate duplication and support Calendly-style service availability. Database schema extended with service_booking_windows table (day-specific availability: Monday 9-12, Wednesday 8-16, etc.) and service_blockers table (recurring time blocks: weekly/monthly/yearly). ServiceAvailabilityService provides complete CRUD with validation for booking windows and blockers, fixing critical monthly/yearly recurrence bug. 11 new REST API endpoints for windows/blockers management with master-role guards. Migration script (src/scripts/migrate-service-restrictions.ts) converts legacy JSONB to normalized tables. Frontend restructure: BusinessDetailsSection updated (Opening Hours = generic info only, NOT booking logic), ServiceConfigSection completely rewritten (457 lines → new structure referencing Services page, booking windows UI, blockers UI, trigger words management), QuestionnaireSection simplified (403 lines → configuration-focused with activation toggle, 6 trigger types, service linking). Admin API client extended with 11 servicesApi methods and questionnaireApi.update. Missing PUT /api/questionnaires/:id endpoint added. Both workflows tested and running cleanly. Architect-approved for production deployment.
