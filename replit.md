...
**LATEST UPDATE (Nov 5, 2025):** **PRODUCTION LOGGING & GPT VERIFICATION SYSTEM** - Implemented comprehensive transparency logging for booking creation (exact calendar data, team member context, buffer times) and GPT interactions (data submitted to AI, customer context verification). Enhanced customer context to include detailed booking records with times and team members. Added strict 8-rule verification system in MASTER_SYSTEM_PROMPT preventing GPT from inventing booking data - bot must reference verified DB records or escalate. Railway logs now show complete end-to-end booking flow and GPT input/output for professional debugging. Previous: Birthday Wishes feature complete with timezone-safe handling, database hotfixes (payment_amount_chf, team_members.color). All architect-approved and production-ready.
