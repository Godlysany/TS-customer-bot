...
**LATEST UPDATE (Nov 5, 2025):** **BIRTHDAY NURTURING FEATURE COMPLETE + DATABASE HOTFIXES** - Completed full Birthday Wishes implementation with timezone-safe date handling across backend, frontend, and scheduler. All birthday-related functions now parse dates locally (no UTC shift). Added 30-day upcoming birthdays preview, fixed setting key alignment (`birthday_wish_template`), and corrected age calculation edge case. Database hotfixes: (1) Corrected `bookings.total_cost` to `bookings.payment_amount_chf`; (2) Added `team_members.color` column backward compatibility. Production-ready and architect-approved.
