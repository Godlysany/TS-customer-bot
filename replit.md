...
**LATEST UPDATE (Nov 5, 2025):** **PRODUCTION DATABASE HOTFIXES** - Fixed critical production errors reported in Railway logs: (1) Corrected `bookings.total_cost` references to `bookings.payment_amount_chf` in customers API and NoShowService penalty calculations; (2) Added `ALTER TABLE` statement to ensure `team_members.color` column exists in existing production databases. Schema now includes backward-compatibility safeguards for smooth migrations. All database column references verified and logs clean.
