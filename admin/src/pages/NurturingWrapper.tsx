...
const NurturingWrapper = () => {
  ...
  useEffect(() => {
    const tabVisibility: Record<string, boolean> = {
      questionnaires: visibility.showQuestionnaires,
      campaigns: visibility.showCampaigns,
      promotions: visibility.showPromotions,
      birthday: visibility.showBirthdayWishes,
      testimonials: visibility.showTestimonials,
      documents: visibility.showDocuments,
      recurring: visibility.showRecurring,
    };

    // If current tab is hidden, switch to first visible tab
    if (!tabVisibility[activeTab]) {
      const firstVisibleTab = Object.entries(tabVisibility).find(([_, visible]) => visible)?.[0];
      if (firstVisibleTab) {
        setActiveTab(firstVisibleTab as typeof activeTab);
      }
    }
