...
import { FileText, Search, Plus, Trash2, Edit, X, Save, Calendar, User, Eye, MessageSquare, Settings, Power, AlertCircle, Link as LinkIcon, PlayCircle, PauseCircle } from 'lucide-react';
...
const QuestionnairesTab = () => {
  ...
  const [activeTab, setActiveTab] = useState<'questionnaires' | 'responses' | 'settings'>('questionnaires');
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [editingQuestionnaire, setEditingQuestionnaire] = useState<Questionnaire | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState('all');
  const [selectedResponse, setSelectedResponse] = useState<any>(null);
  const [selectedQuestionnaireForSettings, setSelectedQuestionnaireForSettings] = useState<string | null>(null);
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async () => {
      if (!selectedQuestionnaireForSettings) return;
      await questionnaireApi.update(selectedQuestionnaireForSettings, settingsConfig);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['questionnaires'] });
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async (active: boolean) => {
      if (!selectedQuestionnaireForSettings) return;
      await questionnaireApi.update(selectedQuestionnaireForSettings, { active });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['questionnaires'] });
  ...
  useEffect(() => {
    if (selectedQForSettings) {
      setSettingsConfig({
        active: selectedQForSettings.active ?? true,
        trigger_type: selectedQForSettings.trigger_type || 'manual',
        linked_services: selectedQForSettings.linked_services || [],
        linked_promotions: selectedQForSettings.linked_promotions || [],
      });
    }
  }, [selectedQForSettings]);

  const toggleService = (serviceId: string) => {
    const linked = settingsConfig.linked_services || [];
    if (linked.includes(serviceId)) {
      setSettingsConfig({ ...settingsConfig, linked_services: linked.filter(id => id !== serviceId) });
    } else {
      setSettingsConfig({ ...settingsConfig, linked_services: [...linked, serviceId] });
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              ...
              To create new questionnaires or edit questions, use the Questionnaire Management tab above.
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {questionnaires.map((q: any) => (
                <button
                  key={q.id}
                  onClick={() => setSelectedQuestionnaireForSettings(q.id)}
                  className={`w-full p-4 border-2 rounded-lg text-left transition-colors ${
                    selectedQuestionnaireForSettings === q.id
            ...
            {questionnaires.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                <p>No questionnaires found. Create one using the Questionnaire Management tab.</p>
              </div>
            )}
          </div>
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <div className="text-sm text-gray-600 mt-1">
                      {selectedQForSettings.active ? 'This questionnaire will be triggered according to its configuration' : 'This questionnaire is paused and will not be sent'}
                    </div>
                  </div>
                  <button
                    onClick={() => toggleActiveMutation.mutate(!selectedQForSettings.active)}
                    disabled={toggleActiveMutation.isPending}
                    className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                      selectedQForSettings.active
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      value={settingsConfig.trigger_type}
                      onChange={(e) => setSettingsConfig({ ...settingsConfig, trigger_type: e.target.value })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                    >
                  ...
                  {(settingsConfig.trigger_type === 'post_service' || settingsConfig.trigger_type === 'first_booking') && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      <div className="space-y-2">
                        {services.map((service: any) => (
                          <label key={service.id} className="flex items-center gap-2">
                            <input
                              type="checkbox"
                              checked={settingsConfig.linked_services.includes(service.id)}
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  <div className="text-sm text-gray-600">
                    <strong>Name:</strong> {selectedQForSettings.name}
                  </div>
                  <div className="text-sm text-gray-600">
                    <strong>Description:</strong> {selectedQForSettings.description}
                  </div>
                  <div className="text-sm text-gray-600">
                    <strong>Total Questions:</strong> {selectedQForSettings.questions?.length || 0}
                  </div>
                  <div className="mt-4">
                    <div className="text-sm font-medium text-gray-700 mb-2">Questions:</div>
                    <ol className="list-decimal list-inside space-y-1 text-sm text-gray-600">
                      {selectedQForSettings.questions?.map((q: any, i: number) => (
                ...
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-xs text-yellow-800">
                    ...
                    To edit questions or create new questionnaires, use the Questionnaire Management tab above.
                  </p>
                </div>
              </div>
            </>
